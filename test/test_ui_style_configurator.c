...
static int
style_rgba(struct nk_context *ctx, const char *name, struct nk_color *color)
{
  nk_label(ctx, name, NK_TEXT_LEFT);
  if (nk_combo_begin_color(ctx, *color, nk_vec2(nk_widget_width(ctx), 400))) {
    nk_layout_row_dynamic(ctx, 120, 1);
    *color = nk_rgba_cf(nk_color_picker(ctx, nk_color_cf(*color), NK_RGBA));

    nk_layout_row_dynamic(ctx, 25, 1);
    color->r = nk_propertyi(ctx, "#R", 0, color->r, 255, 1, 1);
    color->g = nk_propertyi(ctx, "#G", 0, color->g, 255, 1, 1);
    color->b = nk_propertyi(ctx, "#B", 0, color->b, 255, 1, 1);
    color->a = nk_propertyi(ctx, "#A", 0, color->a, 255, 1, 1);
...
{
  style_rgba(ctx, name, &item->data.color);
}
...
style_vec2(struct nk_context *ctx, const char *name, struct nk_vec2 *vec)
{
  ...
  if (nk_combo_begin_label(ctx, buffer, nk_vec2(200, 200))) {
    nk_layout_row_dynamic(ctx, 25, 1);
    nk_property_float(ctx, "#X:", -100.0f, &vec->x, 100.0f, 1.0f, 0.5f);
    nk_property_float(ctx, "#Y:", -100.0f, &vec->y, 100.0f, 1.0f, 0.5f);
    nk_combo_end(ctx);
  }
}
...
style_global_colors(struct nk_context *ctx, struct nk_color color_table[NK_COLOR_COUNT])
{
  ...
  for (i = 0; i < NK_COLOR_COUNT; ++i) {
    clicked |= style_rgba(ctx, color_labels[i], &color_table[i]);
  }
...
style_text(struct nk_context *ctx, struct nk_style_text *out_style)
{
  ...
  nk_layout_row_dynamic(ctx, 30, 2);
  style_rgba(ctx, "Color:", &text.color);

  style_vec2(ctx, "Padding:", &text.padding);
  nk_property_float(ctx, "#Color Factor:", 0.0f, &text.color_factor, 1.0f, 0.1f, 0.0025f);
  nk_property_float(ctx, "#Disabled Factor:", 0.0f, &text.disabled_factor, 1.0f, 0.1f, 0.0025f);

  *out_style = text;
}

static void
style_button(struct nk_context       *ctx,
             struct nk_style_button  *out_style,
             struct nk_style_button **duplicate_styles,
             int                      n_dups)
{
  ...
  style_rgba(ctx, "Border:", &button.border_color);

  style_rgba(ctx, "Text Background:", &button.text_background);
  style_rgba(ctx, "Text Normal:", &button.text_normal);
  style_rgba(ctx, "Text Hover:", &button.text_hover);
  style_rgba(ctx, "Text Active:", &button.text_active);
...
#define BOTTOM_RIGHT  NK_TEXT_ALIGN_BOTTOM | NK_TEXT_ALIGN_RIGHT
    ...
    nk_property_float(ctx, "#Border:", -100.0f, &button.border, 100.0f, 1.0f, 0.5f);
    nk_property_float(ctx, "#Rounding:", -100.0f, &button.rounding, 100.0f, 1.0f, 0.5f);

    nk_layout_row_dynamic(ctx, 30, 1);
    nk_property_float(ctx, "#Color Factor Background:", 0.0f, &button.color_factor_background, 1.0f, 0.1f, 0.0025f);
    nk_property_float(ctx, "#Color Factor Text:", 0.0f, &button.color_factor_text, 1.0f, 0.1f, 0.0025f);
    nk_property_float(ctx, "#Disabled Factor:", 0.0f, &button.disabled_factor, 1.0f, 0.1f, 0.0025f);
...
static void
style_toggle(struct nk_context *ctx, struct nk_style_toggle *out_style)
{
  ...
  style_rgba(ctx, "Border:", &toggle.border_color);
  style_rgba(ctx, "Text Background:", &toggle.text_background);
  style_rgba(ctx, "Text Normal:", &toggle.text_normal);
  style_rgba(ctx, "Text Hover:", &toggle.text_hover);
  style_rgba(ctx, "Text Active:", &toggle.text_active);

  style_vec2(ctx, "Padding:", &toggle.padding);
  style_vec2(ctx, "Touch Padding:", &toggle.touch_padding);

  nk_property_float(ctx, "#Border:", -100.0f, &toggle.border, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Spacing:", -100.0f, &toggle.spacing, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Color Factor:", 0.0f, &toggle.color_factor, 1.0f, 0.1f, 0.0025f);
  nk_property_float(ctx, "#Disabled Factor:", 0.0f, &toggle.disabled_factor, 1.0f, 0.1f, 0.0025f);
...
style_selectable(struct nk_context *ctx, struct nk_style_selectable *out_style)
{
  ...
  style_rgba(ctx, "Text Normal:", &select.text_normal);
  style_rgba(ctx, "Text Hover:", &select.text_hover);
  style_rgba(ctx, "Text Pressed:", &select.text_pressed);
  style_rgba(ctx, "Text Normal Active:", &select.text_normal_active);
  style_rgba(ctx, "Text Hover Active:", &select.text_hover_active);
  style_rgba(ctx, "Text Pressed Active:", &select.text_pressed_active);
  ...
  nk_property_float(ctx, "#Rounding:", -100.0f, &select.rounding, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Color Factor:", 0.0f, &select.color_factor, 1.0f, 0.1f, 0.0025f);
  nk_property_float(ctx, "#Disabled Factor:", 0.0f, &select.disabled_factor, 1.0f, 0.1f, 0.0025f);
...
style_slider(struct nk_context *ctx, struct nk_style_slider *out_style)
{
  ...
  style_rgba(ctx, "Bar Normal:", &slider.bar_normal);
  style_rgba(ctx, "Bar Hover:", &slider.bar_hover);
  style_rgba(ctx, "Bar Active:", &slider.bar_active);
  style_rgba(ctx, "Bar Filled:", &slider.bar_filled);
  ...
  nk_property_float(ctx, "#Bar Height:", -100.0f, &slider.bar_height, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Rounding:", -100.0f, &slider.rounding, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Color Factor:", 0.0f, &slider.color_factor, 1.0f, 0.1f, 0.0025f);
  nk_property_float(ctx, "#Disabled Factor:", 0.0f, &slider.disabled_factor, 1.0f, 0.1f, 0.0025f);
  ...
  if (slider.show_buttons) {
    ...
    /* nk_layout_row_dynamic(ctx, 30, 1); */
    if (nk_tree_push(ctx, NK_TREE_TAB, "Slider Buttons", NK_MAXIMIZED)) {
...
style_knob(struct nk_context *ctx, struct nk_style_knob *out_style)
{
  ...
  style_rgba(ctx, "Border:", &knob.border_color);

  /* knob */
  style_rgba(ctx, "Knob Normal:", &knob.knob_normal);
  style_rgba(ctx, "Knob Hover:", &knob.knob_hover);
  style_rgba(ctx, "Knob Active:", &knob.knob_active);
  style_rgba(ctx, "Knob Border:", &knob.knob_border_color);

  /* cursor */
  style_rgba(ctx, "Cursor Normal:", &knob.cursor_normal);
  style_rgba(ctx, "Cursor Hover:", &knob.cursor_hover);
  style_rgba(ctx, "Cursor Active:", &knob.cursor_active);

  /* properties */
  style_vec2(ctx, "Padding:", &knob.padding);
  style_vec2(ctx, "Spacing:", &knob.spacing);
  nk_property_float(ctx, "#Border:", -100.0f, &knob.border, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Knob Border:", -100.0f, &knob.knob_border, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Cursor Width:", -100.0f, &knob.cursor_width, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Color Factor:", 0.0f, &knob.color_factor, 1.0f, 0.1f, 0.0025f);
  nk_property_float(ctx, "#Disabled Factor:", 0.0f, &knob.disabled_factor, 1.0f, 0.1f, 0.0025f);
...
style_progress(struct nk_context *ctx, struct nk_style_progress *out_style)
{
  ...
  /* TODO rgba? */
  style_rgba(ctx, "Border Color:", &prog.border_color);
  style_rgba(ctx, "Cursor Border Color:", &prog.cursor_border_color);

  style_vec2(ctx, "Padding:", &prog.padding);

  nk_property_float(ctx, "#Rounding:", -100.0f, &prog.rounding, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Border:", -100.0f, &prog.border, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Cursor Border:", -100.0f, &prog.cursor_border, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Cursor Rounding:", -100.0f, &prog.cursor_rounding, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Color Factor:", 0.0f, &prog.color_factor, 1.0f, 0.1f, 0.0025f);
  nk_property_float(ctx, "#Disabled Factor:", 0.0f, &prog.disabled_factor, 1.0f, 0.1f, 0.0025f);

  *out_style = prog;
}

static void
style_scrollbars(struct nk_context          *ctx,
                 struct nk_style_scrollbar  *out_style,
                 struct nk_style_scrollbar **duplicate_styles,
                 int                         n_dups)
{
  ...
  /* TODO rgba? */
  style_rgba(ctx, "Border Color:", &scroll.border_color);
  style_rgba(ctx, "Cursor Border Color:", &scroll.cursor_border_color);

  style_vec2(ctx, "Padding:", &scroll.padding);

  nk_property_float(ctx, "#Border:", -100.0f, &scroll.border, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Rounding:", -100.0f, &scroll.rounding, 100.0f, 1.0f, 0.5f);

  /* TODO naming inconsistency with style_scrollress? */
  nk_property_float(ctx, "#Cursor Border:", -100.0f, &scroll.border_cursor, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Cursor Rounding:", -100.0f, &scroll.rounding_cursor, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Color Factor:", 0.0f, &scroll.color_factor, 1.0f, 0.1f, 0.0025f);
  nk_property_float(ctx, "#Disabled Factor:", 0.0f, &scroll.disabled_factor, 1.0f, 0.1f, 0.0025f);
  ...
  if (scroll.show_buttons) {
    ...
    /* nk_layout_row_dynamic(ctx, 30, 1); */
    if (nk_tree_push(ctx, NK_TREE_TAB, "Scrollbar Buttons", NK_MAXIMIZED)) {
...
style_edit(struct nk_context *ctx, struct nk_style_edit *out_style)
{
  ...
  style_rgba(ctx, "Cursor Normal:", &edit.cursor_normal);
  style_rgba(ctx, "Cursor Hover:", &edit.cursor_hover);
  style_rgba(ctx, "Cursor Text Normal:", &edit.cursor_text_normal);
  style_rgba(ctx, "Cursor Text Hover:", &edit.cursor_text_hover);
  style_rgba(ctx, "Border:", &edit.border_color);
  style_rgba(ctx, "Text Normal:", &edit.text_normal);
  style_rgba(ctx, "Text Hover:", &edit.text_hover);
  style_rgba(ctx, "Text Active:", &edit.text_active);
  style_rgba(ctx, "Selected Normal:", &edit.selected_normal);
  style_rgba(ctx, "Selected Hover:", &edit.selected_hover);
  style_rgba(ctx, "Selected Text Normal:", &edit.selected_text_normal);
  style_rgba(ctx, "Selected Text Hover:", &edit.selected_text_hover);

  style_vec2(ctx, "Scrollbar Size:", &edit.scrollbar_size);
  style_vec2(ctx, "Padding:", &edit.padding);

  nk_property_float(ctx, "#Border:", -100.0f, &edit.border, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Rounding:", -100.0f, &edit.rounding, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Cursor Size:", -100.0f, &edit.cursor_size, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Row Padding:", -100.0f, &edit.row_padding, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Color Factor:", 0.0f, &edit.color_factor, 1.0f, 0.1f, 0.0025f);
  nk_property_float(ctx, "#Disabled Factor:", 0.0f, &edit.disabled_factor, 1.0f, 0.1f, 0.0025f);
...
style_property(struct nk_context *ctx, struct nk_style_property *out_style)
{
  ...
  style_rgba(ctx, "Border:", &property.border_color);
  style_rgba(ctx, "Label Normal:", &property.label_normal);
  style_rgba(ctx, "Label Hover:", &property.label_hover);
  style_rgba(ctx, "Label Active:", &property.label_active);

  style_vec2(ctx, "Padding:", &property.padding);

  /* TODO check weird hover bug with properties, happens in overview basic section too */
  nk_property_float(ctx, "#Border:", -100.0f, &property.border, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Rounding:", -100.0f, &property.rounding, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Color Factor:", 0.0f, &property.color_factor, 1.0f, 0.1f, 0.0025f);
  nk_property_float(ctx, "#Disabled Factor:", 0.0f, &property.disabled_factor, 1.0f, 0.1f, 0.0025f);
  ...
  if (nk_tree_push(ctx, NK_TREE_TAB, "Property Buttons", NK_MAXIMIZED)) {
    dups[0] = &property.dec_button;
    style_button(ctx, &property.inc_button, dups, 1);
    nk_tree_pop(ctx);
  }

  if (nk_tree_push(ctx, NK_TREE_TAB, "Property Edit", NK_MAXIMIZED)) {
...
style_chart(struct nk_context *ctx, struct nk_style_chart *out_style)
{
  ...
  style_rgba(ctx, "Border:", &chart.border_color);
  style_rgba(ctx, "Selected Color:", &chart.selected_color);
  style_rgba(ctx, "Color:", &chart.color);

  style_vec2(ctx, "Padding:", &chart.padding);

  nk_property_float(ctx, "#Border:", -100.0f, &chart.border, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Rounding:", -100.0f, &chart.rounding, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Color Factor:", 0.0f, &chart.color_factor, 1.0f, 0.1f, 0.0025f);
  nk_property_float(ctx, "#Disabled Factor:", 0.0f, &chart.disabled_factor, 1.0f, 0.1f, 0.0025f);
...
style_combo(struct nk_context *ctx, struct nk_style_combo *out_style)
{
  ...
  style_rgba(ctx, "Border:", &combo.border_color);
  style_rgba(ctx, "Label Normal:", &combo.label_normal);
  style_rgba(ctx, "Label Hover:", &combo.label_hover);
  style_rgba(ctx, "Label Active:", &combo.label_active);
  ...
  nk_property_float(ctx, "#Border:", -100.0f, &combo.border, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Rounding:", -100.0f, &combo.rounding, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Color Factor:", 0.0f, &combo.color_factor, 1.0f, 0.1f, 0.0025f);
  nk_property_float(ctx, "#Disabled Factor:", 0.0f, &combo.disabled_factor, 1.0f, 0.1f, 0.0025f);
...
style_tab(struct nk_context *ctx, struct nk_style_tab *out_style)
{
  ...
  style_rgba(ctx, "Border:", &tab.border_color);
  style_rgba(ctx, "Text:", &tab.text);
  ...
  nk_property_float(ctx, "#Indent:", -100.0f, &tab.indent, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Border:", -100.0f, &tab.border, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Rounding:", -100.0f, &tab.rounding, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Color Factor:", 0.0f, &tab.color_factor, 1.0f, 0.1f, 0.0025f);
  nk_property_float(ctx, "#Disabled Factor:", 0.0f, &tab.disabled_factor, 1.0f, 0.1f, 0.0025f);
...
style_window_header(struct nk_context *ctx, struct nk_style_window_header *out_style)
{
  ...
  style_rgba(ctx, "Label Normal:", &header.label_normal);
  style_rgba(ctx, "Label Hover:", &header.label_hover);
  style_rgba(ctx, "Label Active:", &header.label_active);
...
#undef NUM_ALIGNS
  ...
  /* necessary or do tree's always take the whole width? */
  /* nk_layout_row_dynamic(ctx, 30, 1); */
  if (nk_tree_push(ctx, NK_TREE_TAB, "Close and Minimize Button", NK_MAXIMIZED)) {
...
style_window(struct nk_context *ctx, struct nk_style_window *out_style)
{
  ...
  style_rgba(ctx, "Background:", &win.background);

  style_item_color(ctx, "Fixed Background:", &win.fixed_background);

  style_rgba(ctx, "Border:", &win.border_color);
  style_rgba(ctx, "Popup Border:", &win.popup_border_color);
  style_rgba(ctx, "Combo Border:", &win.combo_border_color);
  style_rgba(ctx, "Contextual Border:", &win.contextual_border_color);
  style_rgba(ctx, "Menu Border:", &win.menu_border_color);
  style_rgba(ctx, "Group Border:", &win.group_border_color);
  style_rgba(ctx, "Tooltip Border:", &win.tooltip_border_color);
  ...
  win.tooltip_origin =
    nk_combo(ctx, tooltip_positions, NK_LEN(tooltip_positions), win.tooltip_origin, 25, nk_vec2(200, 200));
  style_vec2(ctx, "Tooltip offset:", &win.tooltip_offset);

  nk_property_float(ctx, "#Rounding:", -100.0f, &win.rounding, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Combo Border:", -100.0f, &win.combo_border, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Contextual Border:", -100.0f, &win.contextual_border, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Menu Border:", -100.0f, &win.menu_border, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Group Border:", -100.0f, &win.group_border, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Tooltip Border:", -100.0f, &win.tooltip_border, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Popup Border:", -100.0f, &win.popup_border, 100.0f, 1.0f, 0.5f);
  nk_property_float(ctx, "#Border:", -100.0f, &win.border, 100.0f, 1.0f, 0.5f);

  nk_layout_row_dynamic(ctx, 30, 1);
  nk_property_float(ctx, "#Min Row Height Padding:", -100.0f, &win.min_row_height_padding, 100.0f, 1.0f, 0.5f);

  if (nk_tree_push(ctx, NK_TREE_TAB, "Window Header", NK_MAXIMIZED)) {
...
int
test_ui_style_configurator(struct nk_context *ctx,
                           ...
                           unsigned int       viewport_height)
{
  ...
  if (nk_begin_titled(
    ...
    if (nk_tree_push(ctx, NK_TREE_TAB, "Text", NK_MAXIMIZED)) {
    ...
    if (nk_tree_push(ctx, NK_TREE_TAB, "Contextual Button", NK_MAXIMIZED)) {
      style_button(ctx, &style->contextual_button, NULL, 0);
      nk_tree_pop(ctx);
    }

    if (nk_tree_push(ctx, NK_TREE_TAB, "Menu Button", NK_MAXIMIZED)) {
      style_button(ctx, &style->menu_button, NULL, 0);
      nk_tree_pop(ctx);
    }

    if (nk_tree_push(ctx, NK_TREE_TAB, "Combo Buttons", NK_MAXIMIZED)) {
      style_button(ctx, &style->combo.button, NULL, 0);
      nk_tree_pop(ctx);
    }

    if (nk_tree_push(ctx, NK_TREE_TAB, "Tab Min/Max Buttons", NK_MAXIMIZED)) {
      dups[0] = &style->tab.tab_maximize_button;
      style_button(ctx, &style->tab.tab_minimize_button, dups, 1);
      nk_tree_pop(ctx);
    }
    if (nk_tree_push(ctx, NK_TREE_TAB, "Node Min/Max Buttons", NK_MAXIMIZED)) {
      dups[0] = &style->tab.node_maximize_button;
      style_button(ctx, &style->tab.node_minimize_button, dups, 1);
      nk_tree_pop(ctx);
    }

    if (nk_tree_push(ctx, NK_TREE_TAB, "Window Header Close Buttons", NK_MAXIMIZED)) {
      style_button(ctx, &style->window.header.close_button, NULL, 0);
      nk_tree_pop(ctx);
    }

    if (nk_tree_push(ctx, NK_TREE_TAB, "Window Header Minimize Buttons", NK_MAXIMIZED)) {
      style_button(ctx, &style->window.header.minimize_button, NULL, 0);
      nk_tree_pop(ctx);
    }

    if (nk_tree_push(ctx, NK_TREE_TAB, "Checkbox", NK_MAXIMIZED)) {
      style_toggle(ctx, &style->checkbox);
      nk_tree_pop(ctx);
    }

    if (nk_tree_push(ctx, NK_TREE_TAB, "Option", NK_MAXIMIZED)) {
      style_toggle(ctx, &style->option);
      nk_tree_pop(ctx);
    }

    if (nk_tree_push(ctx, NK_TREE_TAB, "Selectable", NK_MAXIMIZED)) {
      style_selectable(ctx, &style->selectable);
      nk_tree_pop(ctx);
    }

    if (nk_tree_push(ctx, NK_TREE_TAB, "Slider", NK_MAXIMIZED)) {
      style_slider(ctx, &style->slider);
      nk_tree_pop(ctx);
    }

    if (nk_tree_push(ctx, NK_TREE_TAB, "Knob", NK_MAXIMIZED)) {
      style_knob(ctx, &style->knob);
      nk_tree_pop(ctx);
    }

    if (nk_tree_push(ctx, NK_TREE_TAB, "Progress", NK_MAXIMIZED)) {
      style_progress(ctx, &style->progress);
      nk_tree_pop(ctx);
    }

    if (nk_tree_push(ctx, NK_TREE_TAB, "Scrollbars", NK_MAXIMIZED)) {
    ...
    if (nk_tree_push(ctx, NK_TREE_TAB, "Property", NK_MAXIMIZED)) {
    ...
    if (nk_tree_push(ctx, NK_TREE_TAB, "Combo", NK_MAXIMIZED)) {
      style_combo(ctx, &style->combo);
      nk_tree_pop(ctx);
    }

    if (nk_tree_push(ctx, NK_TREE_TAB, "Tab", NK_MAXIMIZED)) {
